...
class TPWechat extends Wechat
{
	...
	public function record($data=''){
		if(empty($data)) $data=$this->getRevData();
		$log_file= APP_ROOT_PATH.'/public/logger/wx-log-'. date('Y-m-d').'.log';
		error_log('wx-debug :'. PHP_EOL . json_encode($data) .PHP_EOL,3,$log_file);	
	}
	/**
	 * 重载设置缓存
